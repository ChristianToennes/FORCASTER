...
def normalize(images, mAs_array, kV_array, percent_gain):
    ...
    if images.shape[2] > 2000:
        norm_images_gained = np.array([image*(1+gain/100) for image,gain in zip(images[:,edges:-edges:4, edges:-edges:4], percent_gain)])
    else:
        norm_images_gained = np.array([image*(1+gain/100) for image,gain in zip(images[:,edges:-edges:2, edges:-edges:2], percent_gain)])
...
def reg_real_data():
    ...
    for name, proj_path, cbct_path, methods in projs:
        ...
        try:
            ...
            i0s = [i0_est(ims[i], projs[:,i]) for i in range(ims.shape[0])]
            ims = -np.log(ims/np.mean(i0s))
