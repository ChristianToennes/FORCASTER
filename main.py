...
def reg_rough_parallel(ims, ims_big, params, config, c=0):
    ...
    log_queue = mp.Queue()
    log_proc = mp.Process(target=threads.it_log, args=(log_queue,), daemon=True)
    log_proc.start()
            ...
            for _ in range(len(pool), pool_size, 2):
                ...
                for _ in range(4):
                    ...
                    if "profile" in config and config["profile"]:
                        ...
                        name = None
                    proc = mp.Process(target=threads.it_func, args=(p[1], config["Ax_gen"], config["Ax_gen_big"], log_queue, ready, name, config["shm_meta"]), daemon=True)
                    proc.start()
                    proc_count += 1
